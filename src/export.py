...
def export_docx(
    ...
    # Title
    title = doc.add_heading(quiz.title or "测验", level=1)
    ...
    if student_mode:
        doc.add_paragraph("姓名：____________________________  日期：____________")

    # Questions section
    doc.add_heading("题目", level=2)
...
def _add_docx_question(
    ...
    if not image_embedded and not student_mode and nq.get("image_description"):
        p = doc.add_paragraph()
        run = p.add_run(f"建议配图：{nq['image_description']}")
        run.italic = True
        run.font.size = Pt(9)
...
def _add_docx_tf_options(doc, nq: dict, student_mode: bool = False):
    ...
    In teacher mode the correct answer is bolded.
    """
    for opt_letter, val, answer_value in [("A", "正确", "True"), ("B", "错误", "False")]:
        p = doc.add_paragraph()
        run = p.add_run(f"    {opt_letter}. {val}")
        if not student_mode and nq["correct_answer"] in {answer_value, val}:
            run.bold = True
...
def _add_docx_stimulus(doc, nq: dict, student_mode: bool = False):
    ...
    for si, sq in enumerate(sub_qs):
        ...
        if sq_type == "mc" and sq.get("options"):
            ...
            for opt_l, val, answer_value in [("A", "正确", "True"), ("B", "错误", "False")]:
                p = doc.add_paragraph()
                run = p.add_run(f"      {opt_l}. {val}")
                if not student_mode and sq.get("correct_answer") in {answer_value, val}:
                    run.bold = True
...
def export_pdf(
    ...
    if student_mode:
        ...
        c.drawString(50, y, "姓名：____________________________  日期：____________")
        y -= 14
...
def _pdf_info_lines(quiz, style_profile: dict, student_mode: bool = False) -> List[str]:
    ...
    if sol:
        if isinstance(sol, list):
            sol = ", ".join(sol)
        lines.append(f"课程标准：{sol}")

    if not student_mode:
        framework = style_profile.get("cognitive_framework")
        if framework:
            lines.append(f"认知框架：{framework.capitalize()}")

        difficulty = style_profile.get("difficulty")
        if difficulty:
            lines.append(f"难度：{difficulty}/5")

        provider = style_profile.get("provider")
        model = style_profile.get("model")
        if provider:
            generated_by = model if model else provider
            lines.append(f"生成模型：{generated_by}")

    created = getattr(quiz, "created_at", None)
    if created:
        if isinstance(created, datetime):
            lines.append(f"生成日期：{created.strftime('%Y-%m-%d')}")
        else:
            lines.append(f"生成日期：{created}")
...
def _pdf_draw_question(
    ...
    if not image_embedded and nq.get("image_description") and not student_mode:
        ...
        y = _pdf_draw_wrapped_text(c, f"【建议配图：{nq['image_description']}】", 60, y, page_width - 120, page_height)
        c.setFont("Helvetica", 10)
    ...
    if nq["type"] == "mc" and nq["options"]:
        ...
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(70, y, "（请选择所有符合条件的选项）")
        y -= 14
        c.setFont("Helvetica", 10)
        letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        for j, opt in enumerate(nq["options"]):
        ...
        for opt_letter, val in [("A", "正确"), ("B", "错误")]:
            if y < 60:
                c.showPage()
                y = page_height - 50
        ...
        c.drawString(70, y, "作答：" + "_" * 40)
        y -= 14
        # Word bank if available
        word_bank = nq.get("word_bank")
        if word_bank:
            c.setFont("Helvetica-Oblique", 10)
            c.drawString(70, y, "词语库：" + ", ".join(word_bank))
            c.setFont("Helvetica", 10)
            y -= 14
    elif nq["type"] == "short_answer":
        if nq.get("rubric_hint") and not student_mode:
            c.setFont("Helvetica-Oblique", 9)
            c.drawString(70, y, f"（提示：{nq['rubric_hint']}）")
            y -= 14
            c.setFont("Helvetica", 10)
        ...
        if y < 60:
            ...
            y = page_height - 50
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(70, y, "可选项：" + "  |  ".join(definitions))
        c.setFont("Helvetica", 10)
        y -= 14
    elif nq["type"] == "cloze" and nq.get("blanks"):
    ...
    if audio_dir and q_id:
        ...
        if os.path.isfile(audio_path):
            if y < 60:
                ...
                y = page_height - 50
            c.setFont("Helvetica-Oblique", 9)
            c.drawString(70, y, f"【音频：q{q_id}.mp3】")
            c.setFont("Helvetica", 10)
            y -= 14
...
def _pdf_draw_wrapped_text(c, text: str, x: float, y: float, max_width: float, page_height: float) -> float:
    ...
    return pdf_wrap_text(c, text, x, y, max_width, page_height)
...
def generate_pdf_preview(questions, filename, quiz_title, image_map=None):
    ...
    """
    c = configure_pdf_canvas(canvas.Canvas(filename, pagesize=letter))
    width, height = letter
