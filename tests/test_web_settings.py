"""
Tests for TeachFlow LLM provider settings and selection UI.
...
class TestSettingsPage:
    ...
    def test_settings_get_returns_200(self, client):
        ...
        assert "模型服务".encode() in resp.data
    ...
    def test_settings_post_flashes_success(self, client):
        ...
        # base.html renders flashes via {{ message|tojson }}, which escapes
        # non-ASCII text as \uXXXX; compare against the exact JSON form.
        assert json.dumps("设置已保存。", ensure_ascii=True).encode() in resp.data
    ...
    def test_settings_nav_link_exists(self, client):
        ...
        assert b"/settings" in resp.data
        assert 'href="/settings" role="menuitem">设置</a>'.encode() in resp.data
...
class TestGenerateProviderDropdown:
    ...
    def test_generate_form_shows_provider_dropdown(self, client):
        ...
        assert 'for="provider">模型服务'.encode() in resp.data
    ...
    def test_generate_form_shows_default_option(self, client):
        ...
        resp = client.get("/classes/1/generate")
        assert '<option value="">使用默认服务'.encode() in resp.data
...
class TestQuizDetailProvider:
    ...
    def test_quiz_detail_shows_provider(self, client):
        ...
        assert resp.status_code == 200
        assert "生成服务：".encode() in resp.data
        assert b"openai" in resp.data
...
class TestPixabaySettings:
    ...
    def test_settings_shows_not_configured_badge(self, client):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            resp = client.get("/settings")
            assert 'badge-unavailable">未配置<'.encode() in resp.data
    ...
    def test_settings_shows_configured_badge(self, client):
        ...
        with patch.dict(os.environ, {"PIXABAY_API_KEY": "test-key-123"}):
            resp = client.get("/settings")
            assert 'badge-available">已配置<'.encode() in resp.data
...
class TestPixabayTestAPI:
    ...
    def test_test_missing_key(self, client):
        ...
        data = resp.get_json()
        assert data["success"] is False
        assert "需要 API 密钥" in data["message"]
...
class TestUserManagement:
    ...
    def test_add_user_duplicate_rejected(self, admin_client):
        ...
        assert json.dumps("用户名“adminuser”已存在。", ensure_ascii=True).encode() in resp.data
    ...
    def test_add_user_password_mismatch(self, admin_client):
        ...
        assert json.dumps("两次输入的密码不一致。", ensure_ascii=True).encode() in resp.data
    ...
    def test_add_user_password_too_short(self, admin_client):
        ...
        assert json.dumps("密码至少需要 8 个字符。", ensure_ascii=True).encode() in resp.data
